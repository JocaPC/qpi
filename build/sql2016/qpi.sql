...
as
select	text =  IIF(LEFT(query_sql_text,1) = '(', SUBSTRING( query_sql_text, (PATINDEX( '%)[^,]%', query_sql_text))+2, LEN(query_sql_text)), query_sql_text) ,
...
AS
SELECT
		text =   IIF(LEFT(text,1) = '(', SUBSTRING( text, (PATINDEX( '%)[^,]%', text))+2, LEN(text)), text) ,
...
select	t.query_text_id, q.query_id,
		text =   IIF(LEFT(t.query_sql_text,1) = '(', SUBSTRING( t.query_sql_text, (PATINDEX( '%)[^,]%', t.query_sql_text))+2, LEN(t.query_sql_text)), t.query_sql_text) ,
...
SELECT  text =   IIF(LEFT(t.query_sql_text,1) = '(', SUBSTRING( t.query_sql_text, (PATINDEX( '%)[^,]%', t.query_sql_text))+2, LEN(t.query_sql_text)), t.query_sql_text) ,
		params =  IIF(LEFT(t.query_sql_text,1) = '(', SUBSTRING( t.query_sql_text, 2, (PATINDEX( '%)[^,]%', t.query_sql_text+')'))-1), "") ,
		qs.*,
		t.query_text_id
FROM query_stats qs
